...
#              based off a previous model by Walker and Russo (2004).
# How to run: This program takes three arguments in the following order:
...
def train(neu, training_patterns, w, cycles=600):
    ...
    for diag in range(neu):
        w[0][diag][diag] = 0 #self-connection weights set to zero
        w[1][diag][diag] = 0
    return w, prev_o
...
def update_weights(w, prev_o, o, a_s=0.01, a_l=0.0001):
...
def test(weights, testing_data):
    success = 0.0 #measure accuracy
    h_dist = np.zeros(len(testing_data)) #measure hamming distance
    count = 0
...
# Output Hamming distance for each pattern after interference
print("HDs after interference: ", h_dist2)
